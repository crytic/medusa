...
func (c *CryticCompilationConfig) validateArgs() error {
	...
	for _, arg := range c.Args {
		if arg == "--export-format" {
			return errors.Errorf("do not specify `--export-format` within crytic-compile arguments as the solc export format is always used")
		}
		if arg == "--export-dir" {
			return errors.Errorf("do not specify `--export-dir` as an argument, use the BuildDirectory config variable instead")
		}
	}
	return nil
}
...
func (c *CryticCompilationConfig) Compile() ([]types.Compilation, string, error) {
	...
	if err != nil {
		return nil, "", errors.Wrap(err, "failed to delete crytic-compile's export directory prior to compilation")
	}
	...
	for i := 0; i < len(matches); i++ {
		...
		if err != nil {
			return nil, "", errors.Wrapf(err, "failed to read crytic-compile's exported solc data at %s", matches[i])
		}
		...
		if err != nil {
			return nil, "", errors.Wrap(err, "failed to parse crytic-compile's exported solc data")
		}
		...
		for sourceAndContractPath, contract := range solcExport.Contracts {
			...
			if splitIndex == -1 {
				return nil, "", errors.Errorf("expected contract path to be of form \"<source path>:<contract_name>\"")
			}
