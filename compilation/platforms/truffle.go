...
func (s *TruffleCompilationConfig) Compile() ([]types.Compilation, string, error) {
	...
	for i := 0; i < len(matches); i++ {
		...
		if err != nil {
			return nil, "", errors.Errorf("unable to parse ABI for contract '%s'", compiledJson.ContractName)
		}
		...
		if err != nil {
			return nil, "", errors.Errorf("unable to parse init bytecode for contract '%s'", compiledJson.ContractName)
		}
		runtimeBytecode, err := hex.DecodeString(strings.TrimPrefix(compiledJson.DeployedBytecode, "0x"))
		if err != nil {
			return nil, "", errors.Errorf("unable to parse runtime bytecode for contract '%s'", compiledJson.ContractName)
		}
