...
func (c *WeightedRandomChooser[T]) Choose() (*T, error) {
	...
	if len(c.choices) == 0 || c.totalWeight.Cmp(big.NewInt(0)) == 0 {
		return nil, errors.Errorf("could not return a weighted random choice because no choices exist with non-zero weights")
	}
	...
	return nil, errors.Errorf("could not obtain a weighted random choice, selected position does not exist")
}
