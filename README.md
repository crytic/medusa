...
- `fuzzing` defines parameters for the fuzzing campaign:
  ...
  - `blockGasLimit` defines the maximum amount of gas a block's transactions can use (thus defining its max transaction count). It is advised not to change this naively, as a minimum must be set for the chain to operate.
