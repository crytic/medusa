...
type MutatingValueGeneratorConfig struct {
	...
	*RandomValueGeneratorConfig
	...
	// ValueSet contains a set of values which the ValueGenerator may use to aid in value generation.
	ValueSet *ValueSet
}
...
func NewMutatingValueGenerator(config *MutatingValueGeneratorConfig) *MutatingValueGenerator {
	...
	config.ValueSet.AddInteger(big.NewInt(0))
	config.ValueSet.AddInteger(big.NewInt(1))
	config.ValueSet.AddInteger(big.NewInt(-1))
	config.ValueSet.AddInteger(big.NewInt(2))
	return generator
}
...
func (g *MutatingValueGenerator) getMutationParams(inputsLen int) (int, int) {
	...
	mutationCount := g.randomProvider.Int()%((g.config.MaxMutationRounds-g.config.MinMutationRounds)+1) + g.config.MinMutationRounds
...
func (g *MutatingValueGenerator) GenerateInteger(signed bool, bitLength int) *big.Int {
	...
	var inputs []*big.Int
	inputs = append(inputs, g.config.ValueSet.Integers()...)
	if signed {
...
var bytesMutationMethods = []func(*MutatingValueGenerator, []byte, ...[]byte) []byte{
	...
	func(g *MutatingValueGenerator, b []byte, inputs ...[]byte) []byte {
		...
		// it.
		randomByteValue := byte(g.randomProvider.Int() % 256)
		if len(b) > 0 {
			b[g.randomProvider.Int()%len(b)] = randomByteValue
		} else {
	...
	func(g *MutatingValueGenerator, b []byte, inputs ...[]byte) []byte {
		...
		if len(b) > 0 {
			...
			b = append(b, byte(g.randomProvider.Int()%256))
		}
		return b
	},
	...
	func(g *MutatingValueGenerator, b []byte, inputs ...[]byte) []byte {
		...
		// to the end here.
		i := g.randomProvider.Int() % (len(b) + 1)
		if i >= len(b) {
			return append(b, by)
		} else {
...
func (g *MutatingValueGenerator) GenerateBytes() []byte {
	...
	inputs := g.config.ValueSet.Bytes()
	g.randomProviderLock.Lock()
	randomGeneratorDecision := g.randomProvider.Float32()
	g.randomProviderLock.Unlock()
...
var stringMutationMethods = []func(*MutatingValueGenerator, string, ...string) string{
	...
	func(g *MutatingValueGenerator, s string, inputs ...string) string {
		...
		randomRune := rune(32 + g.randomProvider.Int()%95)
		...
		r[g.randomProvider.Int()%len(r)] = randomRune
		return string(r)
	},
	// Flip a random bit
	func(g *MutatingValueGenerator, s string, inputs ...string) string {
...
func (g *MutatingValueGenerator) GenerateString() string {
	// If we have no inputs or our bias directs us to, use the random generator instead
	inputs := g.config.ValueSet.Strings()
	g.randomProviderLock.Lock()
	randomGeneratorDecision := g.randomProvider.Float32()
	g.randomProviderLock.Unlock()
