...
type Corpus struct {
	...
	// successfulSequenceFiles represents a corpus directory with files which describe call sequences that should
	// be used for mutations.
	successfulSequenceFiles *corpusDirectory[calls.CallSequence]

	// revertedSequenceFiles represents a corpus directory with files which describe call sequences that should not be
	// used for mutations.
	revertedSequenceFiles *corpusDirectory[calls.CallSequence]
...
func NewCorpus(corpusDirectory string) (*Corpus, error) {
	...
	corpus := &Corpus{
		...
		successfulSequenceFiles: newCorpusDirectory[calls.CallSequence](""),
		revertedSequenceFiles:   newCorpusDirectory[calls.CallSequence](""),
	...
	if corpus.storageDirectory != "" {
		...
		corpus.successfulSequenceFiles.path = filepath.Join(corpus.storageDirectory, "call_sequences", "mutable")
		err = corpus.successfulSequenceFiles.readFiles("*.json")
		if err != nil {
			return nil, err
		}

		// Read immutable call sequences.
		corpus.revertedSequenceFiles.path = filepath.Join(corpus.storageDirectory, "call_sequences", "immutable")
		err = corpus.revertedSequenceFiles.readFiles("*.json")
		if err != nil {
			return nil, err
		}
...
func (c *Corpus) CallSequenceEntryCount(mutable bool, immutable bool, testResults bool) int {
	count := 0
	if mutable {
		count += len(c.successfulSequenceFiles.files)
	}
	if immutable {
		count += len(c.revertedSequenceFiles.files)
	}
	if testResults {
...
func (c *Corpus) Initialize(baseTestChain *chain.TestChain, contractDefinitions contracts.Contracts) (int, int, error) {
	...
	err = c.initializeSequences(c.successfulSequenceFiles, testChain, deployedContracts, true)
	if err != nil {
		return 0, 0, err
	}

	err = c.initializeSequences(c.revertedSequenceFiles, testChain, deployedContracts, true)
	if err != nil {
		return 0, 0, err
	}

	// Calculate corpus health metrics
	corpusSequencesTotal := len(c.successfulSequenceFiles.files) + len(c.revertedSequenceFiles.files) + len(c.testResultSequenceFiles.files)
...
func (c *Corpus) CheckSequenceCoverageAndUpdate(callSequence calls.CallSequence, mutationChooserWeight *big.Int, flushImmediately bool) error {
	...
	if changed {
		if lastMessageResult.Receipt.Status == types.ReceiptStatusSuccessful {
			// If this a successful sequence, save to successful sequence directory.
			err = c.addCallSequence(c.successfulSequenceFiles, callSequence, true, mutationChooserWeight, flushImmediately)
		} else if lastMessageResult.Receipt.Status == types.ReceiptStatusFailed {
			// If this is a reverting sequence, save to the reverted sequence directory.
			err = c.addCallSequence(c.revertedSequenceFiles, callSequence, true, mutationChooserWeight, flushImmediately)
		}
	}
...
func (c *Corpus) Flush() error {
	...
	err := c.successfulSequenceFiles.writeFiles()
	if err != nil {
		return err
	}
	...
	err = c.revertedSequenceFiles.writeFiles()
	if err != nil {
		return err
	}
