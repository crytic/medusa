...
func getMockSimpleCorpus(minSequences int, maxSequences, minBlocks int, maxBlocks int) (*Corpus, error) {
	...
	for i := 0; i < numSequences; i++ {
		err := corpus.addCallSequence(corpus.mutationTargetSequenceFiles, getMockCallSequence(minBlocks+(rand.Int()%(maxBlocks-minBlocks))), true, nil, false)
		if err != nil {
			return nil, err
		}
	}
...
func TestCorpusReadWrite(t *testing.T) {
	...
	testutils.ExecuteInDirectory(t, t.TempDir(), func() {
		...
		matches, err := filepath.Glob(filepath.Join(corpus.mutationTargetSequenceFiles.path, "*.json"))
		assert.NoError(t, err)
		assert.EqualValues(t, len(corpus.mutationTargetSequenceFiles.files), len(matches))
...
func TestCorpusCallSequenceMarshaling(t *testing.T) {
	...
	testutils.ExecuteInDirectory(t, t.TempDir(), func() {
		...
		for _, entryFile := range corpus.mutationTargetSequenceFiles.files {
			// Marshal the entry
		...
		// Remove all items
		for i := 0; i < len(corpus.mutationTargetSequenceFiles.files); {
			corpus.mutationTargetSequenceFiles.removeFile(corpus.mutationTargetSequenceFiles.files[i].fileName)
		}
		assert.Empty(t, corpus.mutationTargetSequenceFiles.files)
	})
}
