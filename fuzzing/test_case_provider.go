...
type TestCaseProvider interface {
	...
	OnFuzzerStarting(fuzzer *Fuzzer) error
	...
	OnWorkerDeployedContractAdded(worker *FuzzerWorker, contractAddress common.Address, contract *types.Contract) error
	...
	OnWorkerDeployedContractDeleted(worker *FuzzerWorker, contractAddress common.Address, contract *types.Contract) error

	// OnWorkerCallSequenceTesting is called before a fuzzing.FuzzerWorker generates and tests a new call sequence.
	OnWorkerCallSequenceTesting(worker *FuzzerWorker) error

	// OnWorkerCallSequenceTested is called after a fuzzing.FuzzerWorker generates and tests a new call sequence.
	OnWorkerCallSequenceTested(worker *FuzzerWorker) error
	...
	OnWorkerCallSequenceCallTested(worker *FuzzerWorker, callSequence types.CallSequence) ([]ShrinkCallSequenceRequest, error)
}
